...
class ResourceManager:
	...
	def buildReferencedNetworkInstances(self):
		...
		if controllers.resource.ResourceInstance.dockerClient!=None:
		...
		for cpath in p.iterdir():
			if cpath.is_dir():
				...
				if cpath.name != 'baseimage':
					if self.getResourceType(cpath.name)==None:
	...
	def createNewResourceInstance(self, resourceType, name, location, params):
		...
		self.logger.debug('creating new resource instance of type '+resourceType.name+' in location '+location+' with name '+name)
		...
		if resourceType.name=='resource::docker-network::1.0':
			...
			instance=controllers.resource.ResourceInstance.findInstancesByLocation(location, params['networkname'], resourceType.name)
			if instance==None:
			...
			for transition in transitions:
				if transition != None:
					...
					if transition['requestState'] == 'IN_PROGRESS':			
						...
						message = 'Rejecting ' + transitionRequest['transitionName'] + ' on ' + resource.name +' (resourceId: ' + resourceId + ') - a transition is already IN_PROGRESS'
	...
	def getResourceTypeList(self):
		...
		for i in self.resources:
			list.append(i.getResourceOverview())
