...
def findInstances(location, typename):
    ...
    instances = []
    for i in resourceInstances:
        if location is not None and i.location == location:
...
def findInstancesByLocation(location, name, typename):
    ...
    for i in resourceInstances:
        if location is not None and i.location == location:
            if typename is not None and i.type.name == typename:
...
class ResourceInstance:
                ...
                try:
                    ...
                    if networks is not None and len(networks) > 0:
                        ...
                        if ipaddrProp in self.properties:
                            ...
                            self.properties[ipaddrProp] = \
                            self.container.attrs['NetworkSettings']['Networks'][network.name]['IPAddress']

                except docker.errors.APIError:
                    self.logger.error('could not find network ' + value)
    ...
    def runStandardTransition(self, transitionName, properties):
        ...
        if self.type is not None and self.type.lifecyclePath is not None:
            if 'lifecycle' in self.type.lifecyclePath and transitionName in self.type.lifecyclePath['lifecycle']:
                self.logger.debug(
                    'running ' + self.type.lifecyclePath['lifecycle'][transitionName] + ' from lifecycle config')
    ...
    def runOperation(self, transitionName, properties):
        ...
        if transitionName == 'addNetwork':
            ...
            if self.type is not None and self.type.operationsPath is not None:
                if 'operations' in self.type.operationsPath and transitionName in self.type.operationsPath[
                    'operations']:
                    self.logger.debug(
                        'running ' + self.type.operationsPath['operations'][transitionName] + ' from operations config')
