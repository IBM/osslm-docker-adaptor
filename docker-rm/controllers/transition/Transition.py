...
class Transition:
    ...
    def __init__(self, transitionRequest=None):
        ...
        if transitionRequest is not None:
    ...
    def parseTransitionRequest(self):
        self.logger.debug(
            'parsing transition request for transition=' + str(self.transitionName) + ' on resource=' + str(
                self.resourceId))
        ...
        if self.resourceId is not None:
            ...
            if self.transitionName in std:
                ...
                if instance.type.isStandardTransition(self.transitionName):
                    ...
                    if self.transitionName.lower() == 'configure':
                        ...
                        self.logger.debug(
                            'A standard transition should have been processed, ' + self.transitionName + ' is not recognised')
            ...
            if resourceType.isStandardTransition(self.transitionName) is False:
                raise controllers.ResourceManager.InvalidTransitionException(self.transitionName)
    ...
    def getTransitionRequestStatus(self):
        ...
        context = {
            'AsynchronousTransitionResponses': globalConfig.configDescriptor['supportedFeatures'][
    ...
    def runTransition(self):
        ...
        if self.task is not None:
            self.logger.debug('running transition ' + self.transitionName + ' on resource instance ' + str(
                self.task.resourceInstance.resourceId))
            self.task.start()
        else:
            raise Exception()
