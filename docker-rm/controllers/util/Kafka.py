...
class Kafka:
    def __init__(self):
        ...
        if globalConfig.configDescriptor["supportedFeatures"]["asynchronousTransitionResponse"] is True:
            self.logger.debug('trying to configure Kafka')
            try:
                self.logger.debug('kafka is set to Active - trying to create kafka producer on ' +
                                  globalConfig.configDescriptor['properties']['responseKafkaConnectionUrl'])
                self.producer = KafkaProducer(
                    bootstrap_servers=globalConfig.configDescriptor['properties']['responseKafkaConnectionUrl'])
            except Exception as e:
                # self.logger.error(e.__class__.__name__)
                self.logger.error('could not connect to kafka server at ' + globalConfig.configDescriptor['properties'][
                    'responseKafkaConnectionUrl'] + ' no messages will be published')
                self.producer = None
